...
std::string Browser::currentPath = "C:\\";

bool Browser::editPath = false;
...
void Browser::Draw()
{
	...
	for (int i = 0; i < 18; i++)
	{
		...
		std::string file = buttonText.substr(split + 1);
		std::string path = buttonText.substr(0, split);

		// FILE ENTRIES
		if (Window::DrawFileEntry(Vector2(-0.98f, 0.71f - (i * 0.1f)), Vector2(0.49f, 0.8f - (i * 0.1f)), file.c_str(), file.length(), i + 100))
		{
			std::cout << "Open File " << buttonText << std::endl;
		...
		{
			std::cout << "Delete File #" << i << "." << std::endl;
...
void Browser::FileCopy(std::string file, std::string path)
{
	...
	{
		std::cout << msclr::interop::marshal_as<std::string>(e->Message) << std::endl;
		std::cout << "Please run this application with elavated privilleges." << std::endl;
		success = false;
	}
